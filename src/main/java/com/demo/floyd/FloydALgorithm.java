...
//                System.out.print(pre[k][i] + " ");
                System.out.printf("%s ", vertex[pre[k][i]]);
            }
            System.out.println();
            //输出dis数组的一行
            for (int i = 0; i < dis.length; i++) {
//                System.out.print(dis[k][i] + " ");
                System.out.printf("(%s到%s的最短路径是 %d)",vertex[k], vertex[i], dis[k][i]);
        ...
        for (int k = 0; k < dis.length; k++) {
            ...
            for (int i = 0; i < dis.length; i++) {
                ...
                for (int j = 0; j < dis.length; j++) {
                    len = dis[i][k] + dis[k][j]; //求出从i顶点出发经过k中间顶点 到达j顶点距离
